...
- **Database-driven feature flags** — config/settings tables storing flags as JSON (e.g., `site_settings`, `app_config`, `feature_flags` tables) — common in Supabase apps that don't use LaunchDarkly/Unleash. Look for `supabase.from('settings').select()` or similar config table reads that control feature visibility
- **Custom A/B test systems** — tables like `ab_experiments`, `ab_assignments` with variant assignment logic — these are feature flags by another name. Check for experiment tables, variant columns, and assignment queries that split users into treatment groups
- **Admin UI flag toggles** — admin pages that read/write flag values from the database. These are the management interface for database-driven flags — trace which admin routes control which flag values and what downstream behavior changes when a flag is toggled
